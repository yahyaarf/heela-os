...
# =====================================================
# SUPABASE CONFIG FOR REAL SHARED CHAT
# Add these in Streamlit Cloud > App > Settings > Secrets:
# SUPABASE_URL = "https://YOUR_PROJECT_ID.supabase.co"
# SUPABASE_ANON_KEY = "YOUR_SUPABASE_ANON_KEY"
# =====================================================
SUPABASE_URL = st.secrets.get("SUPABASE_URL", "PASTE_SUPABASE_URL_HERE")
SUPABASE_ANON_KEY = st.secrets.get("SUPABASE_ANON_KEY", "PASTE_SUPABASE_ANON_KEY_HERE")

SUPABASE_URL_JS = json.dumps(SUPABASE_URL)
SUPABASE_ANON_KEY_JS = json.dumps(SUPABASE_ANON_KEY)
...
<script>
    ...
    const SUPABASE_ANON_KEY = {SUPABASE_ANON_KEY_JS};
    const supabaseReady = SUPABASE_URL !== "PASTE_SUPABASE_URL_HERE" && SUPABASE_ANON_KEY !== "PASTE_SUPABASE_ANON_KEY_HERE";
    ...
    function renderMessageBubbles(messages) {{
        if (!messages || messages.length === 0) {{
            return `<div class="card">No messages yet. Send the first one.</div>`;
        }}

        return messages.map((m) => `
            <div class="${{m.sender === currentSender ? 'message-mine' : 'message-other'}}">
                <div class="message-name">${{escapeHTML(m.sender || 'Unknown')}}</div>
                <div>${{escapeHTML(m.text || '')}}</div>
                <div class="message-time">${{m.created_at ? new Date(m.created_at).toLocaleTimeString([], {{hour: '2-digit', minute:'2-digit'}}) : ''}}</div>
            </div>
        `).join("");
    }}
    ...
    async function refreshChat() {{
        ...
        if (!supabaseReady) {{
            bubbles.innerHTML = `<div class="card"><b>Supabase not connected.</b><br>Add SUPABASE_URL and SUPABASE_ANON_KEY in Streamlit Secrets.</div>`;
            return;
        }}

        const messages = await fetchMessages();
        bubbles.innerHTML = renderMessageBubbles(messages);
    ...
    function renderChat() {{
        return `
            <h2>Messages</h2>
            <p>This is the real shared chat. Both devices read/write from the same Supabase table.</p>
            <div class="card">
                <label><b>Sender:</b></label>
                <select id="senderSelect" onchange="currentSender=this.value; document.getElementById('senderTop').textContent=this.value; refreshChat();">
                    <option>Yahya</option>
                    <option>heela</option>
                </select>
            </div>
            <div id="chatBubbles">
                <div class="card">Loading messages...</div>
            </div>
    ...
    async function sendMsg() {{
        ...
        if (!sb) {{
            alert("Supabase is not connected yet. Add your Supabase URL and anon key in Streamlit Secrets.");
            return;
        }}
        ...
        if (error) {{
            console.error(error);
            alert("Message failed. Check Supabase table/RLS settings.");
            return;
        }}
    ...
    function openApp(app) {{
        ...
        if (app === 'chat') {{
            ...
            setTimeout(() => {{
                const senderSelect = document.getElementById("senderSelect");
                if (senderSelect) senderSelect.value = currentSender;
                const input = document.getElementById("chatText");
